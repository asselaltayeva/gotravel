...
export default function Navigation() {
  return (
    <nav className="m-auto mb-12 max-w-432">
        <div className="flex justify-between items-center px-21.5 ">
            ...
            <ul className="flex items-center gap-x-10">
                ...
                <li key={link.id} className="group relative flex cursor-pointer items-center gap-x-1.75 font-medium
                transition-all duration-200">
                    <a href={link.href} className="text-grey-600 group-hover:text-black">{link.text}</a>
                    {link.dropdown && 
                    <span className="flex size-6 items-center justify-center">
                        <CaretUp className="size-3.5 fill-grey-600 rotate-180 transition-all duration-250 group-hover:fill-black group-hover:rotate-360"/></span>}
